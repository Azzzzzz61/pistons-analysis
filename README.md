# Detroit Pistons 2024-25赛季数据分析

活塞队2024-25赛季球员表现数据分析项目，重点关注得分效率。
![分析结果](chart.png)

## 模块一：个人球员分析

分析活塞队21名球员的得分与投篮效率（eFG%）关系，识别高价值球员。包括：
- 使用eFG%（有效投篮命中率）评估得分效率
- 分析得分量与投篮效率的关系
- 识别高价值球员

## 使用工具

- Python 3.x
- pandas（数据处理）
- matplotlib（数据可视化）
- Jupyter Notebook

## 主要发现

1. Cade Cunningham得分最高（26.1分），但效率中等（eFG% 0.469）
2. Jalen Duren效率最高（eFG% 0.692），但出手少（11.8分）
3. 球队整体效率偏低，大部分球员eFG%低于0.6

## 数据来源

- 数据：[Basketball Reference](https://www.basketball-reference.com/teams/DET/2025.html)
- 时间：2024-25赛季常规赛
- 球员数量：21人

## 文件说明

- `pistons_analysis.ipynb` - 主分析文件，包含完整代码和分析过程
- `pistons.csv` - 活塞队2024-25赛季球员数据
- `nba_players.csv` - NBA全联盟2024-25赛季球员数据
- `nba_players_2024.csv` - NBA全联盟2023-24赛季球员数据
- `chart.png` - 散点图：得分vs效率
- `pistons_contracts.ipynb` - 球员合同性价比分析，含薪资vs表现散点图
- `pistons_contracts.csv` - 活塞队球员合同数据（来源：Spotrac）

## 扩展分析

## 模块二：横向对比 - 活塞 vs 联盟平均

通过6项关键指标对比，发现活塞队的核心特征：
1. **单打型球队** - 助攻少+命中率持平，球队不靠传导球得分
2. **年轻队伍特征** - 篮板多+失误多，身体出色但决策不成熟
3. **靠出手量得分** - 得分高于联盟，但效率偏低

## 模块三：纵向对比 - 本赛季 vs 上赛季

核心球员全部进步：

| 球员 | PTS变化 | eFG%变化 |
|------|---------|----------|
| Cade Cunningham | +3.4 | +0.021 |
| Jaden Ivey | +2.2 | +0.044 |
| Ausar Thompson | +1.3 | +0.042 |
| Jalen Duren | -2.0 | +0.073 |

**Duren现象：** 得分下降但效率大幅提升，印证了球队"围绕Cade构建进攻"的战术调整。

## 模块四：预测模型 - 用上赛季数据预测下赛季得分

使用sklearn线性回归，特征为MP（上场时间）、FGA（出手次数）、eFG%（效率），预测球员得分。

| 模型 | 样本量 | MAE | R² | 说明 |
|------|--------|-----|----|------|
| 活塞队同赛季 | 21人 | 1.49分 | 0.967 | 样本太少，结果不可信 |
| 全联盟同赛季 | 531人 | 0.66分 | 0.981 | 准确但逻辑有缺陷 |
| 全联盟跨赛季 | 413人 | 2.53分 | 0.799 | 最真实的预测模型 |

**关键结论：** 跨赛季模型预测Cade 2025-26赛季得分25.5分（本赛季真实26.1分）。同赛季模型R²虽高，但用FGA预测PTS本质是循环论证，跨赛季模型才是真正有价值的预测。

## 模块五：球员合同性价比分析

合并Spotrac合同数据与球员表现数据，计算每百万美元得分、防守贡献等指标。识别超值合同（Duren年均487万但防守贡献全队最高）和问题合同（Harris年均2600万但表现平庸、31岁无成长空间）。注：分析基于基础统计数据，无法量化挡拆质量等隐性贡献。
